...
import com.lubin.scala.netty.examples.Server
import com.lubin.scala.netty.examples.block.{ConcurrentBlockStore, BlockStore}
  ...
  def handle(request: Request): Response = request.message match {
    case GetBlock(blockId, offset, length) =>
      store(blockId) match {
        ...
        case Some(data) =>
          Response(
            GetBlockResponse(blockId, data.slice(offset.getOrElse(0), length.getOrElse(data.length))))
      }
